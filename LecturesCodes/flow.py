...
class Flow(nn.Module):
    ...
    def forward(self, z):
        ...
        """
        sum_log_det_J = torch.zeros(z.shape[0], device=z.device)
    ...
    def inverse(self, x):
        ...
        sum_log_det_J: [torch.Tensor]
            The sum of the log determinants of the Jacobian matrices of the inverse transformations.
        """
        sum_log_det_J = torch.zeros(x.shape[0], device=x.device)
...
@torch.no_grad()
def sanity_check_coupling_layer(layer: MaskedCouplingLayer, D: int, device: str = "cpu") -> None:
    """Quick correctness test for a coupling layer.

    Checks that inverse(forward(z)) == z (up to numerical tolerance)
    and that log-det terms cancel.

    Expected output:
      - max_abs_error close to 0 (e.g., < 1e-5)
      - max_abs_logdet_sum close to 0
    """
    layer.eval()
    layer.to(device)

    z = torch.randn(256, D, device=device)
    x, ld = layer(z)
    z_rec, ld_inv = layer.inverse(x)

    max_abs_error = (z - z_rec).abs().max().item()
    max_abs_logdet_sum = (ld + ld_inv).abs().max().item()

    print("[SanityCheck] MaskedCouplingLayer")
    print(f"  max_abs_error      = {max_abs_error:.3e}")
    print(f"  max_abs_logdet_sum  = {max_abs_logdet_sum:.3e}")

def train(model, optimizer, data_loader, epochs, device):
    """Train a Flow model by maximum likelihood (minimize negative log-likelihood)."""
    model.train()
...
if __name__ == "__main__":
    ...
    transformations =[]
    # Note: this script is configured for 2D toy data. We therefore use a simple
    # half/half binary mask of shape (D,) and flip it across coupling layers.
    ...
    # Quick sanity check before training/sampling
    sanity_check_coupling_layer(transformations[0], D=D, device=torch.device(args.device))
    ...
    if args.mode == 'train':
        ...
        # Train model
        train(model, optimizer, train_loader, args.epochs, torch.device(args.device))
